...
fn main() {
    ...
    let conn = anki_enviroment.connect_to_database().unwrap().into_connection();
    ...
    let connection = anki_enviroment.connect_to_database().unwrap();

    let decks = connection.get_all_decks().expect("couldn't get decks");
